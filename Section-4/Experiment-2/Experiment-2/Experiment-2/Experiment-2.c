...
void motors_pin_config(void) {
	...
	motors_dir_ddr_reg |= (1<<motors_RB_pin);	 // set **ONLY** motor pins as output
	motors_dir_ddr_reg |= (1<<motors_LB_pin);
	motors_dir_ddr_reg |= (1<<motors_RF_pin);
	motors_dir_ddr_reg |= (1<<motors_LF_pin);
	motors_dir_port_reg &= ~(1<<motors_RB_pin);			// stop motors initially
	motors_dir_port_reg &= ~(1<<motors_LB_pin);
	motors_dir_port_reg &= ~(1<<motors_RF_pin);
	motors_dir_port_reg &= ~(1<<motors_LF_pin);
}
...
void pwm_pin_config(void){
	...
	motors_pwm_ddr_reg |= (1<<motors_pwm_R_pin);		// set **ONLY** left and right channel pin as output
	motors_pwm_ddr_reg |= (1<<motors_pwm_L_pin);
	motors_pwm_port_reg |= (1<<motors_pwm_R_pin) ;		// enable left and right channel
	motors_pwm_port_reg |= (1<<motors_pwm_L_pin) ;	
}
...
void timer_pwm_init(void) {
	...
	TCCR5A_reg |= ((1<<COMA1_bit)| (1<<WGM0_bit) | (1<<COMB1_bit)) ;
	TCCR5A_reg &= ~((1<<COMB0_bit) | (1<<COMA0_bit) | (1<<WGM1_bit)) ;
	

	// Configure for Phase Correct PWM
	// Set Prescalar to 64
	TCCR5B_reg |= ((1<<CS0_bit) | (1<<CS1_bit) ) ;
	TCCR5B_reg &= ~((1<<CS2_bit)| (1<<WGM2_bit)) ;
